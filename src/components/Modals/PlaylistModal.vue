...
const { playlistCardId } = storeToRefs(contentStore);
...
function createList() {
    ...
    generalStore.createPlaylist(listName.value);
    if (playlistCardId.value) generalStore.togglePlaylistItem(listName.value, playlistCardId.value);
    create.value = false;
    listName.value = "";
}
 ...
 * @param title
 */
function toggle(event: Event, title: string) {
    if (playlistCardId.value) generalStore.togglePlaylistItem(title, playlistCardId.value);
}

function getValue(items: string[]): boolean {
    return !!(playlistCardId.value && items.includes(playlistCardId.value));
}

watch(
    () => playlistCardId.value,
    () => {
        create.value = false;
    },
);
