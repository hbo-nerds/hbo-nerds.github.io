<template>
    <div class="row g-3">
        ...
        <div class="col-sm-3">
            ...
            <div class="d-flex flex-column gap-1">
                <div v-for="(date, idx) in groupedDates" :key="idx" class="pe-3">
                    <input :id="'date-' + idx" v-model="filters.date.range" :value="date.value" class="d-none"
                           name="date-filter" type="radio" @change="contentStore.filter()" :disabled="!date.count">
                    <label :class="[{'text-body-tertiary' : filters.date.range !== date.value}, {'text-decoration-line-through' : !date.count}]" :for="'date-' + idx"
                ...
                <!-- other -->
                <div class="pe-3">
                    <input id="checkOther" v-model="filters.date.range" class="d-none" type="radio"
                           value="other">
                    <label :class="filters.date.range === 'other' ? '' : 'text-body-tertiary'" class="form-check-label text-nowrap small w-100"
                           for="checkOther">
                        Other...
                ...
                <div v-if="filters.date.range === 'other'">
                    <div class="mb-2">
                        ...
                        <input id="afterDate" v-model="filters.date.after" class="form-control form-control-sm"
                               type="date"
                               @change="contentStore.filter()">
                    </div>
                    <div>
                        <label class="form-label small fw-lighter mb-1" for="beforeDate">Voor:</label>
                        <input id="beforeDate" v-model="filters.date.before" class="form-control form-control-sm"
                               type="date"
                               @change="contentStore.filter()">
                    </div>
                </div>
        ...
        <div class="col-sm-3">
            ...
            <div class="d-flex flex-column gap-1">
                <div v-for="(duration, idx) in groupedDuration" :key="idx" class="pe-3">
                    <input :id="'duration-' + idx" v-model="filters.duration" :value="duration.value"
                           class="d-none" type="radio" @change="contentStore.filter()" :disabled="!duration.count">
                    <label :class="[{'text-body-tertiary' : filters.duration !== duration.value}, {'text-decoration-line-through' : !duration.count}]" :for="'duration-' + idx"
                           class="form-check-label text-nowrap small w-100">
        ...
        <div class="col-sm-6">
            ...
            <div class="d-flex flex-column gap-1">
                <div class="pe-3">
                    <input id="hide_no_vod" v-model="filters.vodOnly" class="d-none" name="VOD"
                           type="checkbox" @change="contentStore.filter()">
                    <label class="form-check-label small text-nowrap w-100" for="hide_no_vod"
                ...
                <div class="pe-3">
                    <input id="hide_paywall" v-model="filters.free" class="d-none" name="free"
                           type="checkbox" @change="contentStore.filter()">
                    <label class="form-check-label small text-nowrap w-100" for="hide_paywall"
        ...
        <div class="col-sm-6">
            ...
            <div class="d-flex flex-column gap-1">
                <div class="pe-3">
                    <input id="order-1" v-model="sortOption" class="d-none" type="radio"
                           value="newOld">
                    <label class="form-check-label text-nowrap small w-100" for="order-1" :class="sortOption === 'newOld' ? '' : 'text-body-tertiary'">
                        Upload date (new first)
                        <i v-if="sortOption === 'newOld'" class="bi bi-check-lg ms-2"></i>
                    </label>
                </div>
                <div class="pe-3">
                    <input id="order-2" v-model="sortOption" class="d-none" type="radio"
                           value="oldNew">
                    <label class="form-check-label text-nowrap small w-100" for="order-2" :class="sortOption === 'oldNew' ? '' : 'text-body-tertiary'">
                        Upload date (old first)
                        <i v-if="sortOption === 'oldNew'" class="bi bi-check-lg ms-2"></i>
                    </label>
                </div>
                <div class="pe-3">
                    <input id="order-3" v-model="sortOption" class="d-none" type="radio"
                           value="shortLong">
                    <label class="form-check-label text-nowrap small w-100" for="order-3" :class="sortOption === 'shortLong' ? '' : 'text-body-tertiary'">
                        Duration (shortest first)
                        <i v-if="sortOption === 'shortLong'" class="bi bi-check-lg ms-2"></i>
                    </label>
                </div>
                <div class="pe-3">
                    <input id="order-4" v-model="sortOption" class="d-none" type="radio"
                           value="longShort">
                    <label class="form-check-label text-nowrap small w-100" for="order-4" :class="sortOption === 'longShort' ? '' : 'text-body-tertiary'">
