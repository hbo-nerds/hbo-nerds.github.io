...
export const useContentStore = defineStore("content", {
  ...
  actions: {
    ...
    getSingleCollection(collectionId) {
      return collectionId
        ? this.content.filter((item) => item.collection === parseInt(collectionId))
        : [];
    },
    /**
    ...
    filter() {
      ...
      something.forEach((item) => {
        ...
        let first = arr.shift();
        if (allowedTags.includes(first)) advancedSearch[first] = arr.pop();
      });
    ...
    updateUrl() {
      ...
      if (this.filters.free) search.append("free", this.filters.free);
      if (this.filters.vodOnly) search.append("vodOnly", this.filters.vodOnly);
      if (this.filters.date.range !== "all") search.append("date_weeks", this.filters.date.range);
      if (this.filters.duration !== "all") search.append("duration", this.filters.duration);

      if (search.toString().length)
        router.replace({ query: { filter: search.toString() } }).then((r) => {});
      else router.replace({ query: null }).then((r) => {});
    },
  },
});
